...
visualization_msgs::Marker makeForceMarker(const std::string & ns,
...
visualization_msgs::Marker makeForceNormMarker(const std::string & ns,
                                               double norm,
                                               const std::string & bodyName,
                                               const ros::Time & time,
                                               unsigned int id,
...
{
  rate = (unsigned int ) floor(0.5 / this->gc.configuration().timestep);
  skip = (unsigned int ) floor(1 / (rate * this->gc.configuration().timestep));
...
void ContactForcePublisher::update()
{
  ...
  for(size_t ci = 0; ci < res.contacts.size(); ++ci)
  {
    ...
    mc_rbdyn::Contact contact(robots, c.r1_index, c.r2_index, c.r1_surface, c.r2_surface);
    ...
    if(qp_contact.first != -1)
    {
      ...
      for(unsigned int i = 0; i < r1Points.size(); ++i)
      {
        ...
        force_markers[r1_pub_key].markers.emplace_back(
            makeForceMarker(r1_pub_key, r1Start, r1End, c.r1_body, time, ++id, r1_tf_prefix));
        force_markers[r2_pub_key].markers.emplace_back(
            makeForceMarker(r2_pub_key, r2Start, r2End, c.r2_body, time, ++id, r2_tf_prefix));
      }
      force_norm_markers[r1_pub_key].markers.emplace_back(
          makeForceNormMarker(r1_pub_key, r1F.norm(), c.r1_body, time, ++id, r1_tf_prefix));
      force_norm_markers[r2_pub_key].markers.emplace_back(
          makeForceNormMarker(r2_pub_key, r2F.norm(), c.r2_body, time, ++id, r2_tf_prefix));
    }
  }
  update_ready = true;
}
